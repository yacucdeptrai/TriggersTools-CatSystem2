...
namespace CatDecompiler {
	static class Program {
				...
				for (int i = 0; i < args.Length; i++) {
					...
					case "--extension":
						if (extension != null)
							throw new CommandLineException($"Already specified option \"{arg}\"!");
						if (i + 1 == args.Length)
							throw new CommandLineException($"Expected extension after \"{arg}\"!");
						extension = args[++i];
						break;
					...
					case "--error-level":
						if (errorLevel.HasValue)
							throw new CommandLineException($"Already specified option \"{arg}\"!");
						if (i + 1 == args.Length)
							throw new CommandLineException($"Expected error level after \"{arg}\"!");
		...
		private static bool Decompile(string file) {
			...
			PrintWarning($"{Path.GetFileName(file)} is not a CatSystem2 script file!");
		...
		static void DisplayHelp() {
			...
			PrintOption("-h, -?, --help", "Show help");
			PrintOption("-i <infiles...>", "Input files to decompile (supports wildcards)");
			PrintOption("-o, --output <outdir>", "Output directory for decompiled files");
